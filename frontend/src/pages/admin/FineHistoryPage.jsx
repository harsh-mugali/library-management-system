...
function FineHistoryPage() {
    ...
    const loadFines = async () => {
        const res = await API.get("/fine-history");
        setFines(res.data);
    };

    const filteredFines = filterData(fines, search, ["user", "book"]);
                    ...
                    <tbody>
                        ...
                        {filteredFines.map((fine, index) => (
                            <tr key={index} className="border-b hover:bg-gray-50">
                                <td className="p-3">{fine.user}</td>
                                <td className="p-3 text-center">{fine.book}</td>
